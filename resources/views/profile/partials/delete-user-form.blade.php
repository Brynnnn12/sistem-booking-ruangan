<section class="space-y-6">
    ...
    <x-partials.danger-button x-data=""
        x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion')">{{ __('Delete Account') }}</x-partials.danger-button>
    ...
    <x-partials.modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <div class="mt-6">
                <x-partials.input-label for="password" value="{{ __('Password') }}" class="sr-only" />
